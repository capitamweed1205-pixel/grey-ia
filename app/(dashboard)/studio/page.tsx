...
export default function StudioPage() {
  const tools = [
    ...
    { name: 'STUDIO_PRO', link: '/editor', desc: 'Editor Pro', cat: 'EDIT' },
    ...
    { name: 'VIRAL_LAB', link: '/viral', desc: 'Blueprints TikTok', cat: 'SOCIAL' },
  ];

  return (
    <div style={{ padding: '25px', backgroundColor: '#000', minHeight: '100vh' }}>
      <h2 style={{ fontSize: '24px', fontWeight: '900', fontStyle: 'italic', marginBottom: '25px', borderLeft: '3px solid #fff', paddingLeft: '15px' }}>MODULOS_DE_ESTUDIO</h2>
        ...
        {tools.map(t => (
          <Link key={t.link} href={t.link} style={{ textDecoration: 'none' }}>
            <div style={{ backgroundColor: '#0a0a0a', border: '1px solid #161616', padding: '15px', borderRadius: '15px', height: '100px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <span style={{ fontSize: '7px', color: '#fff', backgroundColor: '#222', padding: '2px 5px', width: 'fit-content', borderRadius: '3px', marginBottom: '5px' }}>{t.cat}</span>
              <h3 style={{ color: '#fff', fontSize: '13px', fontWeight: '900', margin: 0 }}>{t.name}</h3>
              <p style={{ color: '#444', fontSize: '9px' }}>{t.desc}</p>
            </div>
          </Link>
        ))}
      </div>
